...
#
# HYBRID (owner-ratified): config-shape assertions are the ALWAYS-RUN, tool-free, deterministic core.
# An opportunistic LIVE-GATE layer runs the real tool (semgrep/gitleaks/node) on the scaffold as a
# BONUS green-proof when present, and skips-with-reason when absent — it is ADVISORY ONLY and NEVER
# changes the verdict (tools may be absent in a bare conformance env; honest-ceiling pattern).
#
...
# --- Opportunistic live-gate (ADVISORY ONLY — never touches RC). Fixed argv per tool (no eval): a
#     small generic runner registry, not archetype config. Absent tool -> skip-with-reason. ---
# ★ SOLE PER-STACK EXTENSION POINT: the argv here is TS/Node-specific (semgrep/gitleaks/node). KW4/KW8
#   extend THIS registry (add a stack's tool + fixed argv) — the rest of the engine stays stack-agnostic.
live_gate_run() {  # tool scaffold-dir
  _lt="$1"; _ls="$2"
  case "$_lt" in
    semgrep)  semgrep --config p/default --error "$_ls/src" >/dev/null 2>&1 ;;
    gitleaks) gitleaks dir "$_ls" --no-banner >/dev/null 2>&1 ;;
    node)     node --version >/dev/null 2>&1 ;;
    *)        return 3 ;;
  esac
}
live_gate_advisory() {  # uses: livegate, DIR
  [ -n "$livegate" ] || return 0
  [ -z "${KW3_NOLIVE:-}" ] || return 0
  if command -v "$livegate" >/dev/null 2>&1; then
    if live_gate_run "$livegate" "$DIR/scaffold"; then
      echo "          live-gate($livegate): present — ran on scaffold, green (bonus proof)"
    else
      echo "          live-gate($livegate): present — ran on scaffold, non-zero (ADVISORY only; config-shape is the gate)"
    fi
  else
    echo "          live-gate($livegate): absent — skipped-with-reason (config-shape is the deterministic proof; adopter CI runs the live tool)"
  fi
}

# ===========================================================================================
# resolve_mechanism — evaluate the current block (shared vars: id/precond/whenabsent/asserts/
# skipreason/fixhint/livegate) against DIR/CI. Updates counters + RC. Prints the per-mechanism verdict.
# ===========================================================================================
resolve_mechanism() {
  [ -n "$id" ] || return 0
...
'
    ...
    if [ "$_mis" = 0 ]; then
      ...
      GREENS=$((GREENS + 1)); live_gate_advisory
    else
...
reset_block() { id=''; precond=''; whenabsent='skip'; asserts=''; skipreason=''; fixhint=''; livegate=''; }
...
" ;;
  ...
  echo "  run; cannot prove a mechanism no scaffold exercises. Config-shape is the deterministic core;"
  echo "  the live-gate layer is an advisory bonus (skips-with-reason when a tool is absent)."
  if [ "$REDS" -gt 0 ]; then
...
selftest() {
  ...
  # --- Demonstrate the opportunistic LIVE-GATE layer once (advisory; never asserts on tool outcome). ---
  if [ -z "${KW3_INNER:-}" ]; then
    echo "selftest NOTE: live-gate layer (advisory, does not affect the verdict) —"
    sh "$0" --manifest "$MAN" "$FIXTURES/good" 2>&1 | grep 'live-gate(' | sed 's/^ */    /' || true
  fi
...
# adopter's exported tree (already incepted; and running the advisory semgrep/gitleaks live-gate there
# spawns workers that race the export's temp-dir cleanup on overlayfs CI — the observed failure). N/A-
