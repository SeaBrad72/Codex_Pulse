...
| 6 — Evidence and tier flip | In re-review | Re-review found and witnessed RED (4 failed / 21 passed) for K11's equality/code-unit mismatch: an astral code point could skip the UTF-16 cutoff at 73. Both cutoffs now use `>= 73` and return the first 73 UTF-16 code units; focused GREEN is 25/25. Fresh Node 22.13.1 verification: clean `npm ci` with 0 vulnerabilities; lint, type-check, build, packed-bin smoke, hook selftest, Markdown links, and diff check PASS; full tests 76/76 PASS; coverage 81.6% statements, 81.13% branches, 87.5% functions, and 81.39% lines with the validator 100% lock PASS. K12 remains resolved; certification and all acceptance outcomes are unchanged. |
