...
| 6 — Evidence and tier flip | In re-review | Final security review's unbounded stdin/EOF wait (K11, Important/high) and control-byte reflection (K12, Minor/low) are resolved by witnessed TDD; certification remains failed and maturity remains `experimental`. Fresh Node 22.13.1 verification: clean `npm ci`; lint, type-check, build, packaged-bin smoke, hook selftest, diff, and Markdown links PASS; focused 44/44 and full 70/70 tests PASS; coverage 81.6% statements, 81.13% branches, 87.5% functions, 81.39% lines, with the validator 100% lock PASS. Acceptance outcomes are unchanged. |
