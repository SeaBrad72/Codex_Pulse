...
curate_db_backed() {  # MODE(1|0) — 0=non-DB: strip the CI region(s) + remove the copied .db-backed marker; 1=DB-backed: keep
  [ "${1:-1}" = 0 ] || return 0
  strip_db_region .github/workflows/ci.yml
  strip_db_region .gitlab-ci.yml
  rm -f .db-backed
  echo "non-DB archetype: stripped the kit:db-backed CI region + removed the .db-backed marker"
}
  ...
  if [ -f hooks/pre-push ]; then
    ...
    if [ -e "$HOOK_DST" ]; then
      ...
      if cp hooks/pre-push "$HOOK_DST" && chmod +x "$HOOK_DST"; then
        ...
        GUARD_STEP="Local runtime guard installed at .git/hooks/pre-push (force-push/push-to-main; bypass: --no-verify)."
      else
