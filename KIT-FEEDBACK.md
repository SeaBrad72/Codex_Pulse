...
| K8 | Installing the recommended GitHub connector would provide the remote authority needed for AC2/AC4/AC5. | The connector can work with existing installed repositories but exposes no repository-creation or branch-protection operation; no `commitlint-lite` repository exists in its installation, and the local `gh` CLI remains unauthenticated. | blocker | Task 4 / GitHub remote precondition | The owner must create/configure the standalone repository or authenticate `gh` before the agent can push the baseline, require `control-plane-ratification`, open the evidence PR, and observe CI/check state. |
