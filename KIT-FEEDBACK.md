...
| K10 | The exact ordinary command `git push origin main` from a non-main feature state would exercise the pre-push hook and produce its explicit push-to-main refusal. | Local `main` already equaled remote `main`, so Git reported `Everything up-to-date`, supplied no ref update for the hook to evaluate, and exited 0. Remote main remained unchanged, but the required denial did not occur. | high | Task 5 / AC3 history negative proof | AC3 failed honestly: exact command `git push origin main`, exit 0. The test recipe needs an actual pending main ref update to exercise the hook, but manufacturing local-main divergence was outside the approved brief and would weaken the claimed proof. Full transcript: `docs/evidence/ac3-pre-push.txt`. |
...
- During AC3, no inline Codex guard denied `git push origin main`. The universal pre-push floor also did not refuse it at the Git boundary because local and remote `main` were identical, so Git supplied no ref update to the hook and exited 0.
