...
**Relationship to other docs:** The **quality bar** companion to `DEVELOPMENT-PROCESS.md` (the flow) and `ENGINEERING-PRINCIPLES.md` (principles + Definition of Done).
...
The authoritative checklist is in **`ENGINEERING-PRINCIPLES.md`**. A feature is done only when code, tests/evals, CI/CD, docs (incl. RUNBOOK), review/merge, accessibility, and production checks all pass. If any box is unchecked, it isn't done.
...
**Conformance:** a project's pipeline is verified by `conformance/ci-gates.sh <workflow>`, which asserts every required gate is declared (the Definition-of-Done "CI/CD" check, `ENGINEERING-PRINCIPLES.md`). Gates are declared **by id on any CI platform** — GitHub Actions `id: gate-X` steps, GitLab CI `gate-X:` job keys, or a documented mapping for other platforms — because the contract is the gate-ids, not a vendor. See `docs/operations/ci-platforms.md`.
