...
**The adopter-onboarding path is honest end-to-end — the kit reaches maturity stage `release-candidate`.**
*(The first-run walk had been announcing controls it never installed, and the readiness gate green-lit a
tree that carried none of them. Fixed here: the gate now verifies the REAL enforcement surface, per harness.)*

### Fixed
- **`scripts/incept.sh` announced a runtime guard it never installed (F1).** On a greenfield first run the
  script printed guard-active guidance while leaving behind no git repository and no `pre-push` hook — a
  claim with nothing under it. incept now git-inits a greenfield tree and installs the hook *before* it
  states the guard is active, and the statement is branch-honest (installed / brownfield-not-overwritten /
  hooks-absent / init-failed) instead of unconditional.
- **`conformance/inception-done.sh` passed on a tree that was not a git repository and claimed a PreToolUse
  guard for a harness that ignores it (F2/F3).** The readiness gate now asserts the enforcement surface that
  actually exists: a live `.git` repository, an executable `pre-push` hook carrying the guard marker, and a
  harness-aware guard line that claims inline PreToolUse denial ONLY for adapters whose `adapter.json`
  declares `command-guard.level == native` (e.g. `claude-code`). A green run now means the surface EXISTS,
  not that a config file mentions it. **Not a released vulnerability** — these were latent adopter-facing
  bugs (the failure mode was a false *green*, never a bypassed live control), so `### Fixed`, not Security.

### Added
- **`MATURITY.md`** — a standing statement of where the kit is (stage `release-candidate`) and what
  `v1.0.0` requires (a real external adopter). Semver keeps meaning *what changed*, not maturity.
- **Friction-capture templates** — `templates/KIT-FEEDBACK-TEMPLATE.md` and
  `templates/FIELD-REPORT-TEMPLATE.md`, plus a loop-terminal capture obligation in the `build` and
  `operating` skills, so an adopter's friction lands in an artifact instead of evaporating.
- **`conformance/inception-done.sh --selftest`** — a mutation-proven fixture matrix (repo/hook/harness
  negatives) that locks the new surface against regression; registered in `conformance/verify.sh`
  (control-check count 68 → 69).

### Changed
- **README maturity section corrected upward and relocated.** It now names the real vehicle (Relay — the
  first end-to-end product built on the kit), keeps `green ≠ verified`, and states the stage plainly:
  `release-candidate`, with `v1.0.0` gated on an external adopter.
- **`scratchpad/` is now `export-ignore`d and un-tracked (F4).** Thirteen maintainer-only KW27 working
  files had leaked into version control; they are removed from the index (the working tree is untouched)
  and excluded from every adopter `git archive` going forward.

*Honest ceiling: a hookless harness (Codex today) cannot enforce inline PreToolUse denial. The kit gives it
the floor — a git `pre-push` hook plus the CI backstop — and says so, rather than claiming a guard it
cannot run.*

## [3.118.1] - 2026-07-09

**KW6-A2-f3 — ratification fork-PR parity: the job honors the exit-0 contract it asserts.**
*(Follow-on to KW6-A2, v3.118.0, PR #280. The `control-plane-ratification` gate documents itself
as "exits 0 regardless — the check-run carries the state," but two `gh` calls could still redden the
build on a fork pull request, where `GITHUB_TOKEN` is read-only regardless of the job's `permissions:`.)*

### Fixed
- **The `gate-agent-boundary` job — which posts the `control-plane-ratification` check-run — no longer
  reddens the build when either `gh` call fails** (`.github/workflows/ci.yml`). The check-run POST's rc is
  captured under `set +e` and reported (never silently swallowed); the `gh pr view` review read is guarded
  inside an `if !` so a failing command substitution cannot trip `bash -e`. Behaviour now matches the
  comment it always carried. **This is not a security fix and admitted nothing:** the failure mode was
  fail-CLOSED — a red build, never a merged PR. On a fork PR the required check simply stays absent, which
  *blocks* the merge. The spurious red shipped in every prior version, so this is a real released-behaviour
  fix — hence `### Fixed`, not `### Security`.
- **Ratification signal — a whitespace-only review line no longer counts as an approval** *(scope addition
  beyond the approved design, disclosed here rather than shipped silently)*. The final approver filter's
  `grep -q .` matched a line of one-or-more spaces or a tab (the `.` matches a space), which would set the
  non-author-approval signal on whitespace; it is now `grep -q '[^[:space:]]'`. It is unreachable through the
  real data path (GitHub logins carry no whitespace; a deleted author renders as the literal string `null`),
  so this is defense-in-depth. The change is **strictly narrowing** — it can only move a verdict toward
  *unratified*, never toward *ratified* — and the `grep -vxF -- "$AUTHOR"` fixed-string / whole-line / `--`
  hardening is unchanged.

### Notes
- **Adopter configuration (load-bearing).** Branch protection must require the **check-run name**
  `control-plane-ratification`, not the job key `gate-agent-boundary`. On a fork PR the job now exits green
  where it previously exited red; a repository that keyed branch protection on the *job* signal rather than
  the posted *check-run* would lose the fork block. This repo requires the check-run name (verified against
  the live API), so the fail-closed ceiling holds.

## [3.118.0] - 2026-07-09

**KW6-A2 — the presence gate: work in flight must reach the board.** *(The presence half of KW6 —
the actual K1 finding, "it didn't track anything until I told it." KW6-A shipped the evidence half in
v3.116.0; this closes the follow-on the owner's KW6-A GO was conditional on.)*

### Added
- **A PR-time backlog-presence merge-gate** (`.github/workflows/ci.yml`, new `backlog-presence` job;
  `conformance/backlog-presence.sh`, new). A PR whose **derived** change-class is `control-plane` or
  `sensitive` cannot merge unless its PR number appears in the `PR` cell of a row on the project's
  `BACKLOG.md`. Like `control-plane-ratification`, it posts an `action_required` check-run and **exits 0**
  — it blocks the merge without reddening the build; an `ordinary`-class PR is N/A. The change-class is
  derived (reconciling `agent-boundary.sh --state`, the union-aware authority, with
  `promotion-readiness.sh --class`), never self-asserted, and **fails closed** — an unreadable
  change-set or unavailable seam is treated as gated, never as ordinary. Binding is to the `PR` **cell**
  (not "somewhere in the row"), matched as a whole token, so `#28`/`#2800`/a `Notes` mention never satisfy
  `#280`.
- **The kit adopts its own flow board** (`BACKLOG.md`, new; `CLAUDE.md` now declares
  `Backlog backend: BACKLOG.md`), so **both** halves of KW6 gate this repo. Seeded with live `Ready` rows
  (KW6-B, KW6-D, KW21) and this slice's own `In Review` row bearing its PR number — the slice gates
  itself.
- `conformance/backlog-lib.sh` (new): the shared board-parser helpers, extracted from
  `conformance/backlog-current.sh` (whose `--selftest` stays byte-identical) so the two backlog checks read
  **one** definition of "a row" and cannot drift apart.

### Changed
- `.gitattributes` `export-ignore`s `BACKLOG.md` (and `SPARKWRIGHT-CONSOLIDATED-BACKLOG.md`), and
  `scripts/adopter-export.sh` carves the kit's `Backlog backend` declaration out of the **exported**
  `CLAUDE.md`. Together these keep `scripts/incept.sh` stamping each adopter their **own** board and keep
  an adopter tree from declaring a backend it does not ship — the kit's own tree is unchanged.

### Honest ceiling
- The gate proves a row bearing the PR number exists in a `PR` cell. It does **not** prove the row describes
  the work, that its state is accurate, or that a human (rather than the agent) put it there — the
  defense is the legible board diff a reviewer reads. And the AMBER **tagless-clone dry-run structurally
  cannot exercise this gate** (there is no PR number in a clone): a green `verify.sh --require` attests the
  `--selftest` ran, never that the live gate works. Its only pre-merge proof is the check's negative
  fixtures plus the slice gating its own PR once opened.

## [3.117.0] - 2026-07-09

### Added
- **The non-vacuity sweep is now a PR-time CI gate** (`.github/workflows/ci.yml`, new `non-vacuity` job).
  It previously ran only weekly in `drift-watch.yml` — which blocks nothing and is `export-ignore`d, so
  no adopter ever had it. A check that passes while evaluating nothing now fails the PR that introduces it.
- `conformance/non-vacuity.sh --only <check.sh>` — judge one control check in seconds instead of the
  ~6-minute full sweep. A zero-match `--only` exits 2; a bare sweep over an empty control set exits 1.
  CI always runs the full sweep.
- `conformance/verify.sh`: new `promotion-actuate-run` control check (66 → 67). The actuate lock's
  `wiring()` function was previously invoked by nothing — dead code — and now gates.

### Fixed
- **Two shipped conformance checks were VACUOUS**: their `--selftest` passed even against a version of
  the check rendered unable to fail. `conformance/script-disclosure.sh` (shipped vacuous in 3.115.0) and
  `conformance/promotion-actuate-wired.sh` (vacuous since 3.102.0). Both had the same root cause —
  non-parameterized discovery meant the selftest could not drive the real-run function (`scan()` /
  `wiring()`) and tested the leaf beneath it, leaving the wrapper's accumulator untested. Both now take
  their targets **as arguments** and are driven by their selftests, with one negative per accumulator.
- `conformance/script-disclosure.sh`: a scan finding zero in-scope scripts printed `OK … 0 in-scope
  scripts` and returned 0. It now fails closed — a scan that evaluated nothing must never report success.

## [3.116.0] - 2026-07-09

**KW6-A — the backlog carries its evidence.** *(Half of KW6's "backlog actually runs" — the evidence
layer. The **presence** layer — work in flight must reach the board at all, the actual K1 finding — is
**not** closed here; see KW6-A2.)* The spine skills now record board state transitions
(`design` names the item it satisfies; `plan` moves `Ready → In Progress` with a link; `build` moves
`In Progress → In Review` on PR-open and `In Review → Done` on landing), and a new lock proves it.
**`conformance/backlog-current.sh`** enforces **state-appropriate evidence**: every row in a non-terminal
board state carries the evidence appropriate to *that* state — `In Progress`→`Links`, `In Review`→`PR`
(**no `N/A` escape**: you cannot be in review without a PR), `Blocked`→`Blocked on`+`Since`. `Ready`,
`Released` and `Done` are ungated. Backend-aware (non-`md` backends N/A); a pure-template board N/As so a
freshly-incepted repo stays first-run-green. **`scripts/incept.sh` now stamps the backlog backend it already
prompts for** — without it the gate resolved the unfilled placeholder to *undeclared* and silently never ran.
Plus git hygiene in `skills/build` (pre-branch `git fetch`, conflicting-PR-skips-CI, verify-trunk-after-merge)
and a `**Backlog item:**` trace field on the feature-request template. verify **64 → 66**.

**Honest ceiling:** a green run proves the board is filled and every claimed-active row points at its
evidence — **not** that the state labels are true, that a link points at the *right* PR, that an
`N/A — <reason>` reason is a *good* one, or that a blocked item's age is acceptable. Staleness is made
visible, never adjudicated. **A project that never adds a row is never gated** — which is precisely the
K1 finding ("it didn't track anything until I told it"), and this check is silent on it. Closing that
requires binding a merged PR to a board row (**KW6-A2**). This gate is the *evidence* half, not the
*presence* half. It also **does not fire on the kit itself** (the kit's board is
`SPARKWRIGHT-CONSOLIDATED-BACKLOG.md`, not `BACKLOG.md`), so it is not yet dogfooded.

### Added
- **`conformance/backlog-current.sh`** + 32 selftest cases — the state-appropriate-evidence lock (verify 64→66).
- **`skills/build`** — a `## Git hygiene` section (pre-branch fetch · conflicting PR skips CI · verify trunk after merge).
- **`templates/FEATURE-REQUEST-TEMPLATE.md`** — a `**Backlog item:**` field (requirements→backlog trace).

### Changed
- **`scripts/incept.sh`** — stamps the chosen backlog backend into the project `CLAUDE.md`, mirroring the
  four sibling stamps. The prompt already captured it; the answer was being discarded.
- **`skills/{design,plan,build}`** — terminal states record the backlog state transition (additive only;
  all conformance markers preserved).

### Fixed
- A **dead gate on the default path**: an unstamped `Backlog backend` field (and a `[` anywhere in a filled
  one) resolved to *undeclared* → N/A, so the board check could never fire for an adopter while CI stayed green.
- A **renamed board column** silently disabled the gate while its success message asserted the board traced;
  an absent gated column is now a schema violation.

## [3.115.0] - 2026-07-09

**KW9-remainder — closes KW9.** Three independent parts. **KW9-A (naming/positioning): closed as satisfied** — the README already conveys the full audience spectrum with no "Vibe"; ratified, no change. **KW9-C (script self-disclosure):** a new standard (every human-run kit script discloses **what it does · what it changes · guardrails**) enforced by `conformance/script-disclosure.sh` (selftest + a real-scan of the kit's own scripts — dogfooded); all **25** kit scripts conformed with the two header lines (comments only, no logic change). **KW9-D (deprecation + API-versioning):** `DEVELOPMENT-STANDARDS.md` §9 thickened — versioned-from-day-one, additive-only, breaking→new-version; announce → maintain → remove-on-major with a migration path. Doc-budget ratified **330→345** (STANDARDS 339/345). verify **62 → 64**.

**Honest ceiling:** the script-disclosure gate proves the two labels are PRESENT, not that their prose is accurate — review owns accuracy. KW9-A/D are governance (no build-ahead gate).

### Added
- **`conformance/script-disclosure.sh`** + 3 fixtures — the disclosure gate (selftest + real-scan; verify 62→64).
- **`DEVELOPMENT-STANDARDS.md`** — the script self-disclosure standard + the §9 deprecation/API-versioning discipline.

### Changed
- **`scripts/*` (25)** — each gains `# What it changes:` / `# Guardrails:` header lines (comments only).
- **`conformance/doc-budget.sh`** — DEVELOPMENT-STANDARDS.md 330→345, total 930→945 (ratified). **`.github/workflows/ci.yml`** — script-disclosure self-test step.


## [3.114.0] - 2026-07-08

**KW9-B — Harness/model neutrality + the decision-integrity engine.** Applies the neutrality standing requirement to the harness axis (instance #3) and cashes the KW19 engine hand-off. **(1) Engine factored** — `conformance/decision-integrity.sh` is now one config-driven engine (axis table: stack/deploy/harness); the three `*-decision-integrity.sh` scripts are 2-line shims. Stack + deploy migrated **behavior-preserving** (byte-identical `--selftest` + real-path output — verified by a pre/post diff). **(2) Harness neutrality** — comparable cards + **honest maturity** in `docs/operations/harness-adapters.md` (only `claude-code` is *verified*; `generic` is *floor-verified*; `gemini`/`codex`/`cursor` are **experimental — declared against the boundary contract but not exercised end-to-end, i.e. unproven, not "supported"**), a `#### Harness fit rationale` field in the project-CLAUDE template, an anti-bias gate (`conformance/harness-decision-integrity.sh`, engine instance #3), and an `incept` fit-vs-maturity disclosure. **(3) Model = subsumed by harness** — no standalone model axis/gate (model-family is a harness fit dimension; the model-id single-source seam is KW15 cleanup, not a neutrality gate). Recorded in the recipe + `DEVELOPMENT-STANDARDS.md` §1 (net-zero).

**Honest ceiling:** the harness gate proves a fit reason is articulated + non-bias-appeal, not that the harness is optimal (Go/No-Go owns that); the maturity tiers are a truth-in-advertising statement (exercised, not declared). The engine refactor changes structure, not verdicts.

### Changed
- **`conformance/decision-integrity.sh`** (new engine) + `stack-`/`deploy-decision-integrity.sh` reduced to shims — behavior-preserving.
- **`DEVELOPMENT-STANDARDS.md`** §1 — harness = instance #3, model subsumed (net-zero 330).
- **`docs/operations/harness-adapters.md`** — cards + maturity + fit rubric. **`scripts/incept.sh`** — harness fit-vs-maturity disclosure + fit-rationale stamp. **`templates/PROJECT-CLAUDE-TEMPLATE.md`** — `#### Harness fit rationale` field. **`.github/workflows/ci.yml`** — explicit engine self-test step.

### Added
- **`conformance/harness-decision-integrity.sh`** + 4 fixtures — harness anti-bias gate (verify 60 → 62; the harness axis records its decision in the shared `CLAUDE.md`, so a missing `#### Harness fit rationale` heading is N/A — kit, adopter export, and pre-Inception adopter alike — enforcing once the heading is present).
- **`docs/adoption/neutrality-by-construction.md`** — engine "now built" (KW9-B) + "Model — subsumed by harness".


## [3.113.0] - 2026-07-08

**KW19 — Neutrality by construction (standing requirement).** Elevates the twice-proven neutrality pattern (stack #1, deploy-target #2) from a described *pattern* to a **standing requirement** every concretization axis must satisfy, and closes a latent false-PASS in the shipped anti-bias gates. **(1)** `DEVELOPMENT-STANDARDS.md` §1 reworded net-zero (330/330) from "pattern" to "standing requirement", pointing at a new recipe. **(2)** New recipe `docs/adoption/neutrality-by-construction.md` — the four obligations, comparable-card template (by-example), the anti-bias-gate checklist, the load-bearing **vocabulary rule** (a fit token must not be a substring of a word in the artifact's own prose), and the engine hand-off to KW9. **(3) Word-boundary vocab fix** in both `conformance/{stack,deploy}-decision-integrity.sh`: the fit-dimension match now left-anchors tokens `(^|[^a-zA-Z])(…)`, closing a latent false-PASS where a bias-only rationale mentioning "HTML/XML" passed on the bare token `ml` inside "html" (and replaces deploy's ad-hoc `edge` anchor with the generic wrap). New load-bearing fixture `substring-bias-adr.md`. **(4)** Engine factoring **deferred to KW9** (instances #3/#4 = harness/model), recorded in the backlog.

**Honest ceiling:** the standing requirement + checklist are governance (enforced by the design gate + review + KW9 consuming the recipe, not an automated meta-check); the vocab fix is the behavioral teeth — **gate-strengthening only** (fewer false-PASSes, never weakening). No new conformance check (`verify.sh` count unchanged at 60); an existing gate is strengthened.

### Changed
- **`conformance/stack-decision-integrity.sh`** · **`conformance/deploy-decision-integrity.sh`** — left-anchor the fit-dimension vocabulary match (word-boundary); deploy drops its ad-hoc `edge` anchor. Gate-strengthening.
- **`DEVELOPMENT-STANDARDS.md`** §1 — pattern → standing requirement (net-zero reword) + pointer to the recipe.

### Added
- **`docs/adoption/neutrality-by-construction.md`** — the neutrality-by-construction recipe: four obligations, card template, anti-bias-gate checklist, vocabulary rule, KW9 engine hand-off.
- **`conformance/fixtures/stack-decision-integrity/substring-bias-adr.md`** — load-bearing negative (the ml-in-html false-PASS).


## [3.112.0] - 2026-07-08

**KW5 — Inception engages the real forks.** Stops Inception silently defaulting on two forks and lifts the KW4-L1 Neutrality pattern to deploy-target (instance #2 → de-risks KW19). **(1) Solo↔team** is now an engaged, announced choice in `scripts/incept.sh` (`--team solo|team`, default solo) — stamped into the project `CLAUDE.md` governance field, with migration guidance (the T2 `enforce_admins` flip); no new gate (control-plane-ratification already enforces team review). **(2) Deploy-target** gains the full pattern: topology **cards** + a fit **rubric** + steer-away + fit-vs-maturity disclosure + an **anti-bias rule** in `docs/adoption/DEPLOYMENT-ENVIRONMENT.md` ("exercised-ness is not a fit reason" — kills the Railway default), a new **anti-bias gate** `conformance/deploy-decision-integrity.sh` (+6 fixtures) on RUNBOOK §4's `#### Deploy-target fit rationale` (registered `--selftest` [non-vacuity-swept] + real-path; 58 → 60 control-checks), and RUNBOOK-template fields. **(3)** deploy-target + solo/team are surfaced at Inception; the stack notice now names fit AND maturity. `DEVELOPMENT-STANDARDS.md` §1 records deploy-target as **instance #2** of the Neutrality pattern (net-zero reword).

**Honest ceiling:** the deploy lint proves a fit reason is articulated + non-exercised-appeal, not that the platform is correct (Go/No-Go owns that). Solo/team engagement surfaces + records the choice; SoD stays server-side (`enforce_admins`). The deploy real-path check is N/A in the kit (no root RUNBOOK) and enforcing in adopters.

### Added
- **`conformance/deploy-decision-integrity.sh`** + 6 fixtures — deploy anti-bias gate (58 → 60).
- **`docs/adoption/DEPLOYMENT-ENVIRONMENT.md`** — topology cards, fit rubric, steer-away, anti-bias, instance-#2 xlink.
- **`scripts/incept.sh`** — `--team solo|team` fork + deploy-target notice; stack notice names maturity.

### Changed
- **`templates/RUNBOOK-TEMPLATE.md`** §4 — `#### Deploy-target fit rationale` + `#### Maturity acknowledged`.
- **`templates/PROJECT-CLAUDE-TEMPLATE.md`** — governance (solo/team) field.
- **`START-HERE.md`** — deploy-target rubric pointer + solo/team decide-at-Inception lead.
- **`DEVELOPMENT-STANDARDS.md`** §1 — Neutrality pattern now names deploy-target as instance #2.

## [3.111.0] - 2026-07-08

**KW4-L1 — stack decision-integrity (anti-bias rubric).** The untested half of the stack work (KW8 proved *acting on* a stack choice; this makes the *decision* fit-driven). Fixes the proven-stack recommendation bias: the kit no longer silently defaults to TS/Node because it is the only verified stack. **(1)** a new **anti-bias conformance gate** (`conformance/stack-decision-integrity.sh`) on ADR-000's `## Fit rationale` field — a fit reason must be present, non-empty, non-placeholder, and cite ≥1 fit dimension; a bias-appeal-only rationale ("proven / default / safest") FAILs. Registered `--selftest` (logic, mutation-swept by `non-vacuity.sh`) + real-path (enforces the adopter ADR). **(2)** a **derivation rubric + steer-away table** in `docs/STACK-SELECTION.md` (CPU-bound→go/rust · JVM-enterprise→java/kotlin · data/ML→python/ml · IaC→terraform · pipelines→data-engineering). **(3)** **fit-vs-maturity disclosure** as a required Inception step (START-HERE §2 + ADR-000 `## Maturity acknowledged`). **(4)** stack cards single-sourced in STACK-SELECTION.md + a reverse link from each of the 10 profiles (enforced by `check-links.sh`). **(5)** the reusable **Neutrality pattern (instance #1: stack)** in DEVELOPMENT-STANDARDS.md §1, for KW5 (deploy-target) and KW19 (all axes) to lift.

**Honest ceiling:** the gate proves a fit reason is *articulated and non-bias-appeal* — it **cannot** prove the choice is *correct or optimal*; that stays the reviewer's / owner's Go/No-Go. The real-path check is N/A in the kit repo (its ADR-000 is the unfilled example) and enforcing in adopter repos.

### Added
- **`conformance/stack-decision-integrity.sh`** + 4 fixtures — anti-bias ADR-000 gate (56 → 58 control-checks).
- **`docs/STACK-SELECTION.md`** — derivation rubric, steer-away table, fit-vs-maturity disclosure, stack-card single-source note.
- **`DEVELOPMENT-STANDARDS.md` §1** — Neutrality-by-construction pattern (instance #1: stack).

### Changed
- **`docs/ADR-000-EXAMPLE.md`, `docs/architecture/ADR-000-stack.md`** — `## Fit rationale` + `## Maturity acknowledged` fields.
- **`START-HERE.md` §2** — derive-don't-default rubric pointer + fit-vs-maturity disclosure step.
- **`profiles/<stack>.md` ×10** — one-line reverse link to the stack card.

## [3.110.0] - 2026-07-08

**D1 — "no silent gate-weakening" hardening.** Clears the banked integrity debt as the v1 push opens: three closes of the same failure mode (something silently weakens a required gate and a green run hides it). **(1) dry-run-parity** — the AMBER tagless-clone dry-run recipe (`skills/verification`) now says to `yaml.safe_load` any `ci.yml` an apply.py touches, plus a "dry-run parity" list of the CI-only gates the local dry-run can't see (workflow YAML parse · `adopter-export` exports committed HEAD · `doc-budget`); closes the false-green that cost KW24 a CI cycle. **(2) soft-fail-hardening** — the terraform first-run-green manifest now asserts `absent [-]-soft-fail` on the Checkov policy gate (a fail-closed-by-default tool an adopter could silently neuter); the KW25-family security-Low. **(3) KW10** — dedicated scanner-config files (`.gitleaks.toml`, `.gitleaksignore`, `.semgrepignore`, `.trivyignore`, `.checkov.yaml/.yml`) are now control-plane in the guard across all three matchers (path + `>`-redirect + verb-scan), with an agent-autonomy fixture asserting Write/Edit + `>` + `sed -i` all DENY (a scanner-config allowlist can silently narrow a required gate). Also locks the ratified **v1 cut** (committed separately): v1 = KW4-L1/5/19/9/6/21 + D1; v2 = orchestration-scale (KW8b/12/13), KW23, KW22, KW14a, KW7, KW26.

**Honest ceilings:** dry-run-parity is conductor recipe-guidance (the conductor running it is the control, not a CI gate); soft-fail closes the *documented* `--soft-fail` flag (conftest `--no-fail` banked pending flag-spelling verification; a broad `|| true` append is not per-gate-assertable); KW10 covers *dedicated* scanner-config files (thresholds embedded in shared `pyproject.toml`/`.golangci.yml` are not path-matchable — out of scope). **KW26** (quote-aware guard rescue) stays deferred — its own security-re-reviewed slice.

### Changed
- **`skills/verification/SKILL.md`** — new "Dry-run parity" section (yaml-parse workflows + the CI-only-gate list).
- **`conformance/incept-manifests/terraform.txt`** — `absent [-]-soft-fail` on the policy-as-code block.
- **`.claude/hooks/guard-core.sh`** — scanner-config files are control-plane (all 3 matchers).
- **`conformance/agent-autonomy.sh`** — KW10 fixture (scanner-config mutations DENY across 3 forms; reads ALLOW).

## [3.109.0] - 2026-07-08

**KW24 — prove the AI-native path (full E6 demonstration).** Converts the kit's headline differentiator from provided + structurally-locked to **behaviorally-proven + governance-complete**, exercised against the reference ml sentiment feature. Three layers now hold: **structural** (`eval-harness-wired.sh`, existing), **behavioral** (new `conformance/eval-harness-runs.sh` — actually *runs* `profiles/ml/evals/test_run.py` + both suites + the trace in the kit's own CI, closing a structural-only false-green hole; its `--selftest` neuters a fixture harness's threshold gate and asserts the lock REDs, so a dead/always-green harness cannot pass), and **governance** (filled `profiles/ml/EVAL-PLAN.md` + `AI-SYSTEM-CARD.md`, gated via `eval-ready`/`responsible-ai-ready` pointed at `profiles/ml`). Adds an additive LLM **cost/quality trace** seam to `run.py` (`--trace` → one NDJSON line: suite/judge/n_cases/mean_score/threshold/pass/elapsed_s + judge_calls/tokens/est_cost_usd). Control-checks 55 → 56; doc-checks 16 → 18.

**Honest ceiling:** proves the AI-native path's mechanics execute end-to-end **offline** + the governance artifacts are filled/classified. Does NOT run a live model, prove a quality bar, prove real cost/latency under load, or prove a *live* judge resists every injection — those need an API key and are the adopter's §7 Eval gate. No overclaim on the dimension that most defines the product.

### Added
- **`conformance/eval-harness-runs.sh`** (new control-check) — behavioral execution lock + `--selftest` (anchor + neutered-threshold negative); wired in `verify.sh`, `claims.tsv`, and CI (`--selftest` + a live main-path run).
- **`profiles/ml/EVAL-PLAN.md`**, **`profiles/ml/AI-SYSTEM-CARD.md`** — filled governance artifacts for the reference sentiment feature; gated via `eval-ready`/`responsible-ai-ready` on `profiles/ml`.

### Changed
- **`profiles/ml/evals/run.py`** — additive `--trace` cost/quality NDJSON emission (no-`--trace` behavior byte-identical; offline judges emit zero cost, real cost is the live judge's).
- **`profiles/ml/evals/test_run.py`** — +`TraceEmissionTest` (pure-python, no subprocess).

## [3.108.0] - 2026-07-08

**KW8-remaining — first-run-green for the 3 non-app stacks (terraform · data-engineering · ml).** Completes the 9-stack first-run-green matrix. The KW3 detector gains one manifest per non-app archetype (`conformance/incept-manifests/{terraform,data-engineering,ml}.txt`), authored as pure ci.yml config-shape (these profiles ship no `scaffold/` dir) with KW25 enforcing-value asserts (Trivy `exit-code: '1'`, `--cov-fail-under=80`, eval `--threshold 0.8`). data-engineering's `db-postgres` is the first non-app DB gate to activate LIVE. Ran as the session-as-conductor orchestrator: 3 Engineer subagents fanned out (isolated, metered), conductor-re-verified (confab-proof), dual-reviewed. SAST parity: data-engineering + ml gain first-party bandit rules (`ruff --extend-select S`), matching the python profile; a known-false-positive S603 (trusted-interpreter subprocess in the ml eval harness) is scoped-suppressed so first run stays genuinely green, not just config-shape green.

### Added
- **`conformance/incept-manifests/{terraform,data-engineering,ml}.txt`** — first-run-green manifests for the 3 non-app archetypes (pure ci.yml config-shape).
- **`conformance/verify.sh`** + **`.github/workflows/ci.yml`** — 3 per-profile first-run-green invocations (control-checks 52 → 55).

### Changed
- **`profiles/data-engineering/ci.yml`**, **`profiles/ml/ci.yml`** — first-party SAST parity (`ruff check --extend-select S`), matching the python profile's KW8·Slice1 fix.
- **`profiles/ml/evals/test_run.py`** — scoped `# noqa: S603` (known false positive: trusted interpreter + literal code) so the new SAST gate is green on first run.

## [3.107.0] - 2026-07-08

**KW25 — manifest "assert the enforcing value" guidance (from the KW8-followon retro).** A first-run-green manifest that gates a security mechanism must assert the gate's **enforcing value** (a threshold/deny-flag/scope), not merely that the step is present — a present-but-inert gate (e.g. OWASP `failBuildOnCVSS` at its plugin default of 11) greens a presence-only assert while never failing the build (CVSS ≤ 10). The rule lands in `DEVELOPMENT-STANDARDS.md` §14 with a worked before/after, and the reference manifest grammar points authors at the in-file exemplar. Guidance, not a gate (author discipline, un-gateable). **KW26 (quote-aware guard rescue) was deferred** from this slice: the mandatory security review found a deny-floor bypass in the naive strip; the correct fix (an awk quote-state scanner) is banked for a future, re-reviewed attempt.

### Changed
- **`DEVELOPMENT-STANDARDS.md`** — the "assert the enforcing value, not presence" first-run-green manifest rule (§14) + worked example.
- **`conformance/incept-manifests/typescript-node.txt`** — header note pointing authors at the rule + the in-file exemplar (`sast-scoped`'s `absent --config auto` + `--error .` scope).
- **`conformance/doc-budget.sh`** — ratified core-doc budget bump for the new §14 rule (DEVELOPMENT-STANDARDS.md 320→330, total 920→930), per the anti-re-bloat ratchet's "raise in a ratified PR" path.

## [3.106.0] - 2026-07-08

**KW8 · follow-on — 4 app stacks, instrumented (rust · java-spring · kotlin · dotnet).** The conductor pattern fanned out 4 Engineer subagents (isolated, metered) to detector first-run-green, ran **4× parallel dual review** as a serialization-wall mitigation, and produced the run's real deliverable: an **instrumentation report** measuring the serial-vs-parallel split, guard-friction, and the review-fan-out effect. Security-parity hardening the review surfaced landed for two profiles: java-spring gains a first-party SAST gate (SpotBugs + find-sec-bugs) and a gating OWASP dependency-check (`failBuildOnCVSS=7`) with pinned security plugins; kotlin's OWASP dependency-check now gates (`failBuildOnCVSS=7`). Q2 honesty (`orchestrator-run.sh` relabeled reference+fixture, not the live driver) and the D1 scratchpad-leak cleanup are folded in.

### Added
- **`conformance/incept-manifests/{rust,java-spring,kotlin,dotnet}.txt`** — per-stack first-run-green manifests (same grammar as `typescript-node.txt`), each non-vacuity-proven at authoring (mutate an assert → RED → revert).
- **4 new continuous gates** in `conformance/verify.sh` + `.github/workflows/ci.yml` — the detector runs each of the 4 profiles on every push.
- **`docs/architecture/2026-07-08-kw8-followon-run-report.md`** — the instrumentation report (timeline, serial:parallel ratio, guard-friction, review-fan-out effect).

### Changed
- **`profiles/java-spring`** — added a SpotBugs + find-sec-bugs SAST gate (`gate-sast`), pinned it and the OWASP/CycloneDX plugins via `<pluginManagement>`, and set `-DfailBuildOnCVSS=7` so the dependency-check actually gates (default 11 = never-fails).
- **`profiles/kotlin`** — set `dependencyCheck { failBuildOnCVSS = 7.0f }` so the OWASP scan gates.
- **`scripts/orchestrator-run.sh`** — header relabeled harness-neutral reference + fixture selftest, not the live driver (Q2 honesty).

### Fixed
- **D1 scratchpad leak** — removed the tracked `scratchpad/kw8` that KW8's squash left on `main` (`git rm -r --cached`); engineer staging no longer lands on the branch.

## [3.105.0] - 2026-07-08

**KW8 · Slice 1 — first orchestrator run (go + python stack parity).** The kit's multi-agent orchestrator ran **live for the first time**: the session-as-conductor fanned out two Engineer subagents into isolated git worktrees (go, python), metered each through the runaway-guard, and serial-merged the results — proving assignment · worktree isolation · the serial merge queue · the runaway-STOP (a deterministic ceiling breach + escalation). Both stacks reach detector first-run-green and are now continuously gated.

### Added
- **`conformance/incept-manifests/go.txt`** + **`conformance/incept-manifests/python.txt`** — per-stack first-run-green manifests (same grammar as `typescript-node.txt`), each with a load-bearing non-vacuity negative demonstrated at authoring time (mutate an assert → RED → revert).
- **`.github/workflows/ci.yml`** + **`conformance/verify.sh`** — the detector now runs against `profiles/go` and `profiles/python` in CI and as registered control-checks (+2, 46→48).
- **`profiles/python/scaffold/pyproject.toml`** — added flake8-bandit (`"S"`) to the ruff lint gate (with an `S101` per-file-ignore for tests), giving python a SAST layer at parity with go (gosec) and TS (semgrep) — a gap the dual review caught; the manifest asserts it.

### Changed
- Manifest hardening from review: bound trivy fail-closed values (`exit-code`, `severity`, `ignore-unfixed`) in both image-supply-chain blocks; tightened the secret-scan assert so a `./subdir`-narrowed scan can't pass; de-tautologized python `agent-trace-emit`.

### Honest ceiling
- The detector proves **config-shape** first-run-green (the tool-free deterministic core), not a live language-gate run. Advisory live-gate registry entries and the `docs/STACK-SELECTION.md` tier-wording bump are **deferred** (documented follow-on). Full KW4 reference-slice thickness (authed CRUD + background job + flag floor) and the remaining 7 stacks are the KW8 follow-on.

## [3.104.0] - 2026-07-07

**Actuation-division correction (fast-follow).** The kit's own process docs *lagged* the shipped model: S6 delegated control-plane actuation to the agent on a recorded GO, and the promotion contract's Tier-3 rule documents it — but `skills/build`/`skills/plan` still said "hand it to a human to apply" and the contract carried an obsolete bootstrap caveat, which kept over-loading the human with mechanical keystrokes. This aligns the docs with the model. **Docs-only — no capability or gate change.**

### Changed
- **`docs/governance/promotion-contract.md`** — deleted the obsolete "during the S1 bootstrap the maintainer still applies by hand" caveat; added the canonical division: on a recorded GO the **agent** actuates all mechanical steps (apply, commit, push, PR, tag, record, check); the **human** does the GO and — solo, control-plane only — the single `gh pr merge --admin` (kill-switch + SoD-substitute). Ordinary/team: the agent merges too → the human does zero mechanical keystrokes.
- **`skills/build/SKILL.md`** + **`skills/plan/SKILL.md`** — the AMBER guidance now says the agent authors the brick + `apply.py` **and, on the recorded GO, runs it**; the human does the GO + the solo `--admin` merge. "Never silently commits control-plane" preserved (never *without* a recorded GO).
- **`conformance/promotion-contract-documented.sh`** — new `require` (`agent-actuates-mechanical`) + a load-bearing `--selftest` negative, so the correction can't silently regress to "hand it to a human."

### Honest ceiling / preserved
No relaxation: the guard still blocks the agent's Write/Edit to control-plane paths **and** the `--admin` bypass; the agent never actuates **without** a recorded GO; `builder ≠ ratifier` and "the GO stays the human's" are unchanged and re-stated. This aligns documentation with an already-shipped capability; it cannot force runtime adherence (the un-gateable `skills/build` ceiling).

## [3.103.0] - 2026-07-07

**KW3 — Inception first-run green.** The dominant dogfood friction (K7 family): a fresh `incept` shipped an archetype's mechanisms *placed but not activated* — commented-out / unscoped / ungated — so the first CI run was a wall of misconfigured reds (an expert works around them; the real external adopter bounces). KW3 delivers a **stack-agnostic detector** + the TS/Node activations. AMBER control-plane.

### Added
- **`conformance/incept-first-run-green.sh`** (new control-check) + a companion **`incept-first-run-green-profile`** registration (44 → 46) — a **stack-agnostic** detector: for an archetype it asserts every expected mechanism (per a per-archetype manifest in `conformance/incept-manifests/`) is **green-or-legibly-skipped**, never a misconfigured red. Config-shape assertions are the deterministic core; an opportunistic live-gate layer runs the real tool (semgrep/gitleaks) when present. **Non-vacuity `--selftest`:** a liveness anchor + N/A-vs-skip + 5 mutated-fixture negatives (one per mechanism) + a **lock self-negative** (a neutered always-green detector FAILS its own selftest). Registered in `conformance/verify.sh` **twice** — `--selftest` (detector integrity) **and** against the live `profiles/typescript-node` (so re-inerting the shipped profile can't stay green forever) — plus `conformance/claims.tsv` and both CI steps. **The reusable engine KW4/KW8 point at the other 9 stacks — add a manifest, run the same lock.**
- **`conformance/incept-manifests/typescript-node.txt`** — the TS/Node expected-mechanism manifest.

### Changed
- **`profiles/typescript-node/ci.yml`** + **`scripts/incept.sh`** — **three** mechanisms are now **ACTIVATED** (they were inert) and **two** are **ASSERTED-AND-LOCKED against regression** (they were already correct — KW3 adds the detector assertion + a clarifying comment, not a behavioral change):
  - **Activated — Postgres** live (delimited `kit:db-backed` region; `incept --no-db` strips the region + the `.db-backed` marker for a non-DB archetype — now guarded so a missing closing marker never wipes gates to EOF) with `DATABASE_URL`.
  - **Activated — SAST** exclusion-scoped: `semgrep --config p/default` scans the whole app (`--error .`) so a new code root (`app/`, `server/`, `lib/`, `pages/`) stays covered by default; a shipped `scaffold/.semgrepignore` opts OUT of only generated/vendored noise (`dist/`, `coverage/`, `node_modules/`). **No narrowing of any adopter's coverage** — the reverse of the old `src/`-only inclusion scope, which silently dropped SAST on any code outside `src/`. Pinned ruleset kept (never `--config auto`).
  - **Activated — agent-trace** auto-emitted in CI (the agentops-sensor path).
  - **Asserted-and-locked — secret-gate** (already correct; unchanged): `gitleaks dir . --no-banner --redact` full working-tree scan, `fetch-depth: 0`; build output is out of scope because it is **`.gitignore`d** (`dist/`, `coverage/`), which `gitleaks dir` respects — **no allowlist file needed** (the former `scaffold/.gitleaksignore` was inert — gitleaks' allowlist holds finding fingerprints, not path globs — and is **removed**). KW3 locks this against regression; it does not change it.
  - **Asserted-and-locked — provenance** (already correct; unchanged): the attestation jobs were already gated (`if:` on repo visibility) to **skip with a legible reason** on a solo private repo — never a red, never a fake green. KW3 locks this against regression; it does not change it.

### Honest ceiling
- The detector proves the **shipped archetype's** gates are green-or-legibly-skipped on first run; it **cannot** prove a mechanism no scaffold exercises (an adopter who later adds payments hits wiring the lock never saw). It converts "we hope it's five" into "the lock catches every instance in the archetypes we run it against." KW3 fixes **TS/Node only**; the generalization to the other 9 stacks is KW8/KW4 (the same lock + a manifest per stack). Domain features (payments/email/uploads) are the adopter's, added green via the normal loop.

## [3.102.0] - 2026-07-07

**S6 — the capstone: the control-plane actuation capability grant.** The agent may now actuate a control-plane promotion on a recorded GO — via one auditable `actuate` gate — bounded by the assurance bar `[authenticated: <forge>-review]` **AND** approver ≠ author, then verifying `shipped == approved`. The solo kill-switch holds **server-side**: a *normal* agent merge is rejected without a real forge review and `--admin` is human-only — the local authenticated-label bar is defense-in-depth + audit over a self-authorable note (the forge-review derivation is a deferred seam). **No gate reads `lean`/`enterprise`** (mode-enforcement-blind preserved). AMBER control-plane.

### Added
- **`scripts/promotion-verify.sh actuate`** (new mode) — the control-plane actuation gate: it merges ONLY on a recorded GO note binding the exact approved SHA whose derived `approved-by:` label is `[authenticated: <forge>-review]` (read from that line **only** — never a body scan) **and** whose approver ≠ the commit author (`builder ≠ ratifier`), via a swappable `--merge-cmd` (default `gh pr merge --squash`, **never `--admin`**), then verifies `shipped == approved`. Fails closed on every gap. Fixture-proven (1 liveness anchor + 9 load-bearing negatives).
- **`conformance/promotion-actuate-wired.sh`** (new control-check, 43 → 44) — wiring + a non-vacuity `--selftest`: the liveness anchor + 9 negatives + guard fixtures + a **LOCK SELF-NEGATIVE** (a neutralized bar makes the lock itself FAIL). Registered in `conformance/verify.sh`, `conformance/claims.tsv`, and CI (`.github/workflows/ci.yml`).

### Changed
- **`.claude/hooks/guard-core.sh`** — (a) `scripts/promotion-verify.sh` is now **control-plane-immutable** (added to `is_control_plane_path` + the command/redirect matchers) so the gate script can no longer be silently weakened, while a bare `sh …/promotion-verify.sh actuate` stays runnable; (b) `gh pr merge --admin`/`--administrator` is denied as a **bypass speed-bump** (the honest boundary stays credential-side: never issue the agent an admin token).
- **`docs/governance/promotion-contract.md`** — the Tier-3 rule is now **operative** (points at `actuate`); the assurance bar + kill-switch-by-construction + honest ceilings are spelled out.
- **`docs/operations/runtime-guards.md`** — the `--admin` speed-bump + the credential-side honesty boundary.
- **`docs/adoption/vc-hosts.md`** — the forge-review → `[authenticated: <forge>-review]` derivation seam sharpened (the label `actuate` consumes as the control-plane bar).

### Honest ceiling
- The gate is **fixture-proven solo**; the live `gh pr merge`, the forge-review→`[authenticated]` derivation, and the team merge credential are **documented seams** (no solo consumer — the repo rests solo). The server-side `--admin` bypass stays **un-guardable** — the guard deny is a speed-bump; the boundary is never issuing the agent an admin credential. S6 itself lands under the current bootstrap (a human applies + `--admin`-merges this PR).

## [3.101.0] - 2026-07-07

**S5b — the recovery net: the control-plane revert is now DRILLED (prove `git revert` restores the last-good tree, reusing the promotion `check`'s tree-equality — no new revert command) and the agent break-glass procedure is assembled from existing primitives. The S6 precondition: you don't hand the agent control-plane actuation until you can undo a mistake fast and have PROVEN the undo works. AMBER control-plane; no capability change.**

### Added
- **`conformance/control-plane-revert-drill.sh`** (new control-check, 42 → 43) — on a git fixture it applies a bad control-plane change over a known-good state G, `git revert`s it, and asserts (a) the reverted tree **equals** G's tree via the same `scripts/promotion-verify.sh check` tree-equality that proves `shipped == approved` (**no parallel verifier, no new revert command**) and (b) a conformance-shaped marker is green. **Load-bearing negative:** a *non-restoring* revert — marker restored green but extra unapproved content rides on top, so the tree differs from G — MUST FAIL; a stub that skips the tree-equality assertion passes the broken case, so the assertion is proven load-bearing. Registered in `conformance/verify.sh` and wired into CI (`.github/workflows/ci.yml`).
- **`docs/operations/break-glass.md`** (new) — the agent break-glass runbook: **halt → revert → rotate → postmortem**, assembled by cross-reference (not duplication) from `scripts/runaway-guard.sh` (resource halt), the drilled control-plane revert, `docs/operations/progressive-delivery.md` (deploy rollback), the DR `docs/continuity/backup-restore-drill.md` (data), `docs/operations/containment.md` §2/§3 (rotate/SoD), and `scripts/postmortem.sh`. **Honest ceiling stated plainly:** the HALT is resource-based (`runaway-guard`) **+ manual** — there is **no bad-action auto-detector**; solo, the human is the monitor.

### Changed
- **`docs/governance/promotion-contract.md`** — a short recovery reference: control-plane recovery reuses this contract's own `check` (revert restores the last-good tree), drilled by `control-plane-revert-drill.sh`, and links the break-glass runbook. Matrix / delegable-execution tiers untouched.

### Honest ceiling
- The drill proves **restoration** now (reverted tree == last-good + a representative green), CI-gateable with no deploy — the concrete S6 precondition. It does **not** prove a deploy rollback *in anger* (needs a real staging environment — **deferred to KW23**), and the break-glass HALT is resource-based + manual, **not** a bad-action auto-detector.

## [3.100.0] - 2026-07-07

**S5a — record hardening: the GO record moves from an in-tree `promotion-log.md` append to a tree-invariant `git notes` binding (`refs/notes/promotions`), and `approved-by` becomes assurance-labeled (`[signed: gpg]` → `[committer]` → `[self-asserted]`, never overclaiming). Fixes S4's two findings; precondition for S6. NON-control-plane only; control-plane stays human-actuated.**

### Changed
- **`scripts/promotion-verify.sh`** — `record` now BINDS the structured GO record to the approved commit as a git note under `refs/notes/promotions` (tree-invariant: it can never perturb the approved tree, so `check` can never false-fail because of the record — closes S4-finding #1). `approved-by` carries a **derived** assurance label (`[signed: gpg]` if the approved-sha is signed → `[committer]` if the approver matches the git committer identity → `[self-asserted]` otherwise) — derived from the commit's evidence, **never** accepted from input, and it never overclaims (an unsigned commit can never be `[signed: gpg]`). New **`log`** subcommand renders the notes as a human-readable projection; `check`/`resolve` read the note. `check`'s tree-equality core is UNCHANGED.
- **`conformance/promotion-verify-wired.sh`** — two new load-bearing teeth: a **tree-invariance** regression (recording a note must not change the approved tree nor dirty the work-tree) and a **label-can't-lie** negative (a smuggled `[signed: gpg]` claim on an unsigned commit is downgraded — the emitted label is never `[signed: gpg]`), plus a note round-trip. The S2 squash / revert / extra-content / missing-content negatives are retained unchanged.
- **`docs/governance/promotion-log.md`** — retired to a pointer (records now live in `refs/notes/promotions`; view with `promotion-verify.sh log`). History kept honest.
- **`docs/governance/promotion-contract.md`** — the Approve→execute→log section documents the notes binding + assurance labels; the honest ceiling now states *notes bind, they do not authenticate*.
- **`docs/adoption/vc-hosts.md`** — defines the **forge-adapter seam** for authenticated approval (GitHub PR review / GitLab MR approval → `[authenticated: <forge>-review]`), wired when a team consumer exists (defer-build-ahead). **Seam only — not wired.**

### Note
- **No new conformance check** (S5a reworks existing checks) — `conformance/verify.sh` stays at 42 control-checks. **No capability change** — control-plane stays human-actuated; the notes binding is a self-authorable *advisory* trail (it binds, it does not authenticate).

## [3.99.0] - 2026-07-07

**Design Cycle 1 complete — the promotion-inversion model: KW1 approve→execute→log + shipped==approved, KW2 demonstrate, KW20 autonomy/lean/kill-switch. The honest actuation model (S4/KW20): the GO is the only validation, the agent executes on the recorded GO, and a human keystroke is never a control — only ever a kill-switch against a compromised/malfunctioning agent. Doc + conformance only; NO capability change (control-plane stays human-actuated — S4 relabels only WHY: a kill-switch + temporary bootstrap, not "human control"; ending it is S6, gated on S5).**

### Changed
- **`docs/governance/promotion-contract.md`** — new *honest actuation model* section (GO = the only validation; the agent executes on the GO; the keystroke is never a control; the two real controls = a second human's independent GO (SoD, team-only) + a compromised-agent kill-switch; `builder ≠ ratifier` preserved as the real SoD); **autonomy** added as a **second modulator** alongside trust (a dial, not a third axis); **`lean` stated as the honest first-class baseline**, `enterprise` as the superset that *adds* scaffolding; the **general kill-switch posture** unifying the deploy-failsafe dial + the solo control-plane keystroke into one circuit-breaker (a posture, not a gate; not a validation). Honest relabel of the retained control-plane keystroke: a **kill-switch + temporary bootstrap**, not "human control". **Matrix cells unchanged.**
- **`conformance/promotion-contract-documented.sh`** — three new load-bearing prose markers (`autonomy-modulator`, `lean-first-class`, `kill-switch-not-validation`), each with an isolatable selftest negative (dropping only its phrase FAILs). **Part C matrix teeth (`assert_cp_cell`) unchanged.**
- **`docs/operations/progressive-delivery.md`** — the general kill-switch posture as an operational posture (default-on where blast-radius is highest + no other defense-in-depth; off when trusted; explicitly not a validation, not a gate), tied to the rollback order.
- **`DEVELOPMENT-PROCESS.md`** — light framing: the honest actuation model, `lean` the honest baseline / `enterprise` the superset, the kill-switch as a posture (no gate reads the mode).

### Note
- **No capability change.** Nothing actuates; control-plane stays human-actuated (bootstrap). All three pieces are documented **principles/postures, not gates** — `conformance/mode-enforcement-blind.sh` stays green (no enforcement-surface script reads the process mode). Ending the control-plane bootstrap (agent actuation capability) is **S6**, gated on **S5** (recovery/rollback + break-glass).

## [3.98.0] - 2026-07-06

**Approve→execute→log (KW1 · S2) — the actuation mechanism: a `shipped == approved` integrity check (merge + tag), an append-only `approved-by:` log, and the `builder ≠ ratifier` invariant. Makes the shipped delegable-execution permission (v3.83.0) auditable and safe for NON-control-plane promotions; control-plane stays human-actuated.**

### Added
- **`scripts/promotion-verify.sh`** (new) — harness-neutral POSIX tool with two modes: `record` (append a structured `approved-by:` entry to the promotion log) and `check` (assert `shipped == approved` — the merged ref / tag carries the approved SHA, and a tag's `VERSION` matches the approved one). A mismatch hard-fails loudly (`SHIPPED != APPROVED`).
- **`docs/governance/promotion-log.md`** (new) — the append-only, git-tracked `approved-by:` record (field spec + one worked example): `approved-sha` · `approved-by` · `gate`/`rung`/`change-class` · `scope` · `approval-token` · `basis`.
- **`conformance/promotion-verify-wired.sh`** (new) — regression lock with a `--selftest` whose **load-bearing negative** builds a throwaway git fixture where the approved SHA is NOT in the shipped ref/tag: `check` MUST exit 1. Registered in `conformance/verify.sh` (control-checks 41 → 42, auto-derived).

### Changed
- **`docs/governance/promotion-contract.md`** — new *Approve→execute→log* subsection (provide means → wait → on the recorded, per-gate, never-inferred GO: execute + log + verify `shipped == approved` at merge AND tag) + the **`builder ≠ ratifier`** invariant (peer to `builder ≠ reviewer`). Uniform delegation riding the existing rung×class gating; control-plane stays human. Matrix cells unchanged.
- **`conformance/promotion-contract-documented.sh`** — four new load-bearing prose markers (`builder-not-ratifier`, `approve-execute-log`, `never-infer`, `shipped-equals-approved`), each with an isolatable selftest negative (dropping only its line FAILs). **Part C matrix teeth (`assert_cp_cell`) unchanged.**
- **`CLAUDE.md`** + **`DEVELOPMENT-PROCESS.md`** — `builder ≠ ratifier` named as a first-class DoD/governance invariant; the `approve→execute→log` protocol referenced.
- **`skills/build/SKILL.md`** + **`skills/review/SKILL.md`** — terminal states reference the actuation handoff (provide means → wait → on recorded GO: actuate + log + verify).

### Note
- **Honest ceiling:** `shipped == approved` is the **gateable** guarantee (record existence + SHA-binding + post-actuation content match are CI-checkable — the half that would have hard-failed this session's tag-on-wrong-commit / content-not-committed slips). `never-infer` is **FLOOR discipline** — that the agent waited + refused to infer approval is not runtime-gateable. Control-plane actuation stays human (bootstrap); the guard matcher (K6/KW11) is out of scope.

## [3.97.0] - 2026-07-06

**Engagement mode (KW2 · S3) — the `demonstrate` skill: surface a working increment for human taste/judgment before a Go/No-Go; advisory teeth (roster-protected + RC surfacing).**

### Added
- **`skills/demonstrate/SKILL.md`** (new) — a first-class, roster-protected kit skill: at a notable increment carrying a **taste-surface** (UI/UX/flow/working-functionality/table, where *correct ≠ good*), produce a demonstrable increment, frame the specific judgment call, and record the verdict in a UAT sign-off (reuses `templates/UAT-SIGNOFF-TEMPLATE.md`). Distinct discipline from `review` (agent→agent correctness) and `verification` (agent→self evidence); a skill an existing seat invokes, not a new seat. Carries the visually-observable-first ordering rule and the KW20 autonomy-density coupling.
- Roster wiring: indexed in `skills/using-skills/SKILL.md`, added to the `conformance/orchestrator-loop-wired.sh` spine table (markers + `skill_path` case + `ORCH_LOOP_DEMONSTRATE_SKILL` env override), and referenced by the Reviewer seat (`agents/reviewer.agent.md`).

### Changed
- **`conformance/promotion-readiness.sh`** — added an **advisory** taste-surface line to the promotion-readiness surfacing (still exit 0; never gates). `skills/build` + `skills/review` terminal states reference `demonstrate`.

### Note
- **Advisory only** this slice: the skill is provided + roster-protected + surfaced at every RC promotion, but no hard "taste-surface touched + no record → fail CI" gate ships here — that rides the KW7 generic obligation-trigger mechanism as a co-follow. Honest ceiling: whether the human actually looks is un-gateable; the FLOOR is *increment demonstrated + judgment framed + verdict recorded*.

## [3.96.0] - 2026-07-06

**Promotion inversion (KW1 · S1) — control-plane actuation delegable on a SHA-bound, per-merge recorded GO; never-weaken invariant; conformance refined (matrix teeth intact).**

### Changed
- **`docs/governance/promotion-contract.md`** — Tier-3 reworded to the **judgment/actuation split**: the control-plane **GO stays human at every rung** (never delegable), while **actuation** (merge/tag/apply) is delegable to the agent **only** on an explicit + recorded + per-merge + **non-blanket** + **SHA-bound** GO (autonomous / inferred / auto-merge / blanket stays banned; SHA-record + post-actuation match-verification = S2). The catastrophic non-promotion set stays denied regardless of any approval.
- **`CLAUDE.md`** + **`DEVELOPMENT-PROCESS.md`** — Definition-of-Done and §13 wording aligned to the judgment/actuation split ("control-plane execution stays human" → "judgment stays human; actuation delegable on a SHA-bound, per-merge recorded GO").
- **`conformance/promotion-contract-documented.sh`** — refined the prose marker (`cp-execution-human` → `cp-judgment-human`) and added a positive marker (`cp-actuation-qualified`) requiring any documented control-plane actuation delegation to name its SHA-bound/per-merge qualifier, plus a load-bearing `cp-blanket` selftest negative (blanket delegation must FAIL). **Part C matrix teeth (`assert_cp_cell`) unchanged** — the Control-plane column stays human-worded at every rung.

### Added
- **Never-weaken invariant** (U14) in the promotion contract: a GO is never reached by weakening security, architecture, or governance.

### Note
- Documentation + conformance only — no new runtime automation. This proves the model is **documented coherently**, not that a gate enforces SHA-bound actuation (that is S2). Honest ceiling.
